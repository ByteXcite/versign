...
def find_signature(im, model):
    ...
    h, w = im.shape
    im = im[h/2:int(h*0.9), int(0.60*w):w]
    w, h = im.shape
...
def extract_signature(im, model):
    x, y, w, h = find_signature(im, model)
    ...
    ret3, cropped = cv2.threshold(cropped, 0, 255, cv2.THRESH_BINARY + cv2.THRESH_OTSU)

    print 'Centering signature'
